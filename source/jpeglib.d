...
enum DCTSIZE             = 8;	/* The basic DCT block is 8x8 samples */
enum DCTSIZE2            = 64;	/* DCTSIZE squared; # of elements in a block */
enum NUM_QUANT_TBLS      = 4;	/* Quantization tables are numbered 0..3 */
enum NUM_HUFF_TBLS       = 4;	/* Huffman tables are numbered 0..3 */
enum NUM_ARITH_TBLS      = 16;	/* Arith-coding tables are numbered 0..15 */
enum MAX_COMPS_IN_SCAN   = 4;	/* JPEG limit on # of components in one scan */
enum MAX_SAMP_FACTOR     = 4;	/* JPEG limit on sampling factors */
 ...
 */
enum C_MAX_BLOCKS_IN_MCU   = 10; /* compressor's limit on blocks per MCU */
enum D_MAX_BLOCKS_IN_MCU   = 10; /* decompressor's limit on blocks per MCU */
...
enum JDCT_DEFAULT = J_DCT_METHOD.JDCT_ISLOW;
enum JDCT_FASTEST = J_DCT_METHOD.JDCT_IFAST;
...
struct jpeg_error_mgr {
   ...
   * end of processing.
   */
  int num_warnings;		/* number of corrupt-data warnings */
   ...
   */
  const(char*) * jpeg_message_table; /* Library errors */
...
enum JPOOL_PERMANENT	= 0;	/* lasts until master record is destroyed */
enum JPOOL_IMAGE	= 1;	/* lasts until done with image/datastream */
enum JPOOL_NUMPOOLS	= 2;

alias jvirt_sarray_control * jvirt_sarray_ptr;
alias jvirt_barray_control * jvirt_barray_ptr;
...
static if (JPEG_LIB_VERSION >= 80){
alias jpeg_core_output_dimensions	jCoreDimensions;
}
...
auto jpeg_create_decompress(j_decompress_ptr cinfo) {
    return jpeg_CreateDecompress(cinfo, JPEG_LIB_VERSION,
			  jpeg_decompress_struct.sizeof);
}
void jpeg_CreateCompress(j_compress_ptr cinfo,
				      int _version, size_t structsize);
...
enum JPEG_SUSPENDED		= 0; /* Suspended due to lack of input data */
enum JPEG_HEADER_OK		= 1; /* Found valid image datastream */
enum JPEG_HEADER_TABLES_ONLY	= 2; /* Found valid table-specs-only datastream */
...
/* Return value is one of: */
/* enum JPEG_SUSPENDED	= 0;    Suspended due to lack of input data */
enum JPEG_REACHED_SOS	= 1; /* Reached start of new scan */
enum JPEG_REACHED_EOI	= 2; /* Reached end of image */
enum JPEG_ROW_COMPLETED	= 3; /* Completed one iMCU row */
enum JPEG_SCAN_COMPLETED	= 4; /* Completed last iMCU row of a scan */
...
enum JPEG_RST0	= 0xD0;	/* RST0 marker code */
enum JPEG_EOI	= 0xD9;	/* EOI marker code */
enum JPEG_APP0	= 0xE0;	/* APP0 marker code */
enum JPEG_COM	= 0xFE;	/* COM marker code */
